...
//! ============> If Api fails then stop the loader
    ...
    //! ============> Call api
    await _getRequest(
      endPoint:  '${NetworkStrings.getChartById}?id=$id',
      context: context,
    );
  ...
  void _getChartByIdSuccessResponse({required BuildContext context}) async {
    try {
      final dataModel = GetChartByIdModel.fromJson(_response?.data);

      print("-----------------");
      print(dataModel.data!.description.toString());
      print("-----------------");
      GetxController.Get.find<HomeController>().chartsById.value = dataModel;
    } catch (e) {
      print(e.toString());

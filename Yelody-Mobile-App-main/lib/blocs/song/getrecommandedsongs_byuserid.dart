...
class GetRecommandedSongByUserId {
  Response? _response;
  VoidCallback? _onSuccess, _onFailure;
  // OtpUserResponseModel? _loginResponse;
  String? deviceToken;

  void loadGetRecommandedSongByUserId ({
...
//! ============> If Api fails then stop the loader
    ...
    //! ============> Call api
    await _getRequest(
      endPoint:  '${NetworkStrings.GetRecommandedSongByUserId}?userId=$userId',
      context: context,
    );
    ...
    _onSuccess = () {
      ...
      AppNavigation.navigatorPop(context);
      _getRecommandedSongByUserIdSuccessResponse(context: context);
    };
  ...
  void _getRecommandedSongByUserIdSuccessResponse({required BuildContext context}) async {
    try {
      final dataModel = GetRecommandedSongByuserIdModel.fromJson(_response?.data);


      GetxController.Get.find<HomeController>().getRecommandedSong.value = dataModel;
